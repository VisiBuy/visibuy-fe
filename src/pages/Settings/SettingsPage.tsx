...
type SettingItem = {
  ...
  route?: string;
  external?: string; // 🔥 NEW — for external links like FAQ
};
    ...
    items: [
      {
        ...
        icon: HelpIconUrl,
        external: "https://visibuy.com.ng/faq", // 🔥 opens in new tab
      },
    ],
  },
];
...
export default function SettingsPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center pb-16 overflow-y-auto h-full">
      {/* Header */}
      <div className="w-full bg-blue-500 text-white py-4 sm:py-5 md:py-6 px-5 sm:px-6 relative flex items-center justify-center">
        <button
          className="absolute left-6 flex flex-col justify-center space-y-[4px] lg:hidden"
          aria-label="Menu"
        >
      ...
      {/* Settings Container */}
      <div className="w-full max-w-[420px] sm:max-w-[540px] md:max-w-[720px] lg:max-w-[900px] xl:max-w-[1100px] px-4 sm:px-6 md:px-8 lg:px-16 xl:px-24 py-6 sm:py-8 md:py-10">
        {settingsItems.map((section) => (
          <div key={section.section} className="space-y-3 mb-8">
            ...
            <div className="flex flex-col gap-3">
              {section.items.map(({ label, subtitle, icon, route, external }) => {
