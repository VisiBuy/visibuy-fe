...
const WHATSAPP_EVENT_MAP = {
  verification_update: {
    ...
    description: "WhatsApp alerts for status updates.",
  },
  escrow_status_change: {
    label: "Escrow Status Changes",
    description: "WhatsApp alerts for payment changes.",
  },
  dispute_notification: {
    label: "Dispute Notifications",
    description: "Urgent WhatsApp for dispute matters.",
  },
};
...
export default function WhatsappNotification({
  ...
  const whatsappToggles = Object.entries(WHATSAPP_EVENT_MAP).map(
    ([key, { label, description }]) => {
      const eventType = key as EventTypeKey;
      // 1. Read the current toggle state from the localSettings
      const isChecked =
        localSettings[eventType]?.[WHATSAPP_CHANNEL_NAME] ?? false;
      ...
      const channelData = preference?.channels.find(
        (c) => c.channel === WHATSAPP_CHANNEL_NAME
      );
      ...
      <div className="opacity-50 pointer-events-none">
        ...
        <div className="text-center font-bold space-y-4 pt-4">
          <p className="text-md text-gray-700">
            Connect WhatsApp: To receive WhatsApp notifications, you need to
